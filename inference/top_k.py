...
def generate_top_k(model, tokenizer, prompt, block_size, max_new_tokens=50, k=50):
    model.eval()
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            # all tokens outside the top K are set to -inf
            # this prevents the model from sampling unlikely tokens
            top_k_logits, top_k_indices = torch.topk(logits, k=k, dim=-1)

            # Build a new logits tensor filled with -inf
            # then place the top K values back in their original positions
            filtered_logits = torch.full_like(logits, float('-inf'))
            filtered_logits.scatter_(1, top_k_indices, top_k_logits)

            # Sample from the filtered distribution
            # k=1 means always take one value, but which one depends on the probabilities
...
if __name__ == "__main__":
    ...
    # small K — conservative, stays close to greedy
    # large K — more diverse, approaches unconstrained sampling
    for k in [1, 10, 50, 100]:
        output = generate_top_k(model, tokenizer, prompt, block_size, 50, k=k)
        print(f"Top-K (k={k}): {output}")