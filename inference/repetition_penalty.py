...
# ─── Repetition Penalty Generation ───────────────────────────────────────────

def generate_repetition_penalty(model, tokenizer, prompt, block_size, max_new_tokens=50, penalty=1.3):
    model.eval()
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            # for each token in the generated sequence so far:
            ...
            # penalty=1.0 means no effect
            # penalty>1.0 discourages repetition
            # penalty<1.0 encourages repetition (rarely useful)
            ...
            # Sample from the penalised distribution
...
if __name__ == "__main__":
    ...
    # penalty=1.0 — no effect, same as standard sampling
    # penalty=1.3 — standard setting, noticeable reduction in repetition
    # penalty=1.5 — aggressive, may produce unusual word choices
    for penalty in [1.0, 1.3, 1.5, 2.0]:
        output = generate_repetition_penalty(model, tokenizer, prompt, block_size, 50, penalty=penalty)
        print(f"Penalty={penalty}: {output}")