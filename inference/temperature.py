...
def generate_temperature(model, tokenizer, prompt, block_size, max_new_tokens=50, temperature=1.0):
    model.eval()
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            # temperature < 1.0 — sharpens the distribution, model becomes more confident
            # temperature > 1.0 — flattens the distribution, model becomes more random
            # temperature = 1.0 — no change, standard softmax
            # temperature → 0   — approaches greedy (always picks the top token)
            # temperature → inf  — approaches uniform random sampling
...
if __name__ == "__main__":
    ...
    for temp in [0.5, 1.0, 1.5, 2.0]:
        output = generate_temperature(model, tokenizer, prompt, block_size, 50, temperature=temp)
